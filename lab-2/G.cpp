#include <iostream>

void merge(std::pair <long long, long long> *a, long long left, long long mid, long long right) {
	long long iterator_1 = 0, iterator_2 = 0;
	std::pair <long long, long long> result [right - left];
	while ((left + iterator_1 < mid) && (mid + iterator_2 < right)) {
		if (a[left + iterator_1].first < a[mid + iterator_2].first) {
			result[iterator_1 + iterator_2].first = a[left + iterator_1].first;
			result[iterator_1 + iterator_2].second = a[left + iterator_1].second;
			iterator_1 ++;
		}
		else {
			result[iterator_1 + iterator_2].first = a[mid + iterator_2].first;
			result[iterator_1 + iterator_2].second = a[mid + iterator_2].second;
			iterator_2 ++;
			}
		}
	while (left +  iterator_1 < mid) {
		result[iterator_1 + iterator_2].first = a[left + iterator_1].first;
		result[iterator_1 + iterator_2].second = a[left + iterator_1].second;
		iterator_1 ++;
	}
	while (mid + iterator_2 < right) {
		result[iterator_1 + iterator_2].first = a[mid + iterator_2].first;
		result[iterator_1 + iterator_2].second = a[mid + iterator_2].second;
		iterator_2 ++;
	}
	for (long long i = 0; i < (iterator_1 + iterator_2); i++) {
		a[left + i] = result[i];
	}
}

void mergeSortRecursive(std::pair <long long, long long> *a, long long left, long long right) {
	if (left + 1 >= right) {
		return;
	}
	long long mid = (left + right) / 2;
	mergeSortRecursive(a, left, mid);
	mergeSortRecursive(a, mid, right);
	merge(a, left, mid, right);
}


int main () {
	long long n, k, y;
	std::cin >> n;
	std::pair <long long, long long> labworks[n];
	for (long long i = 0; i < n; i++) {
		std::cin >> k >> y;
		labworks[i].first = k;
		labworks[i].second = y;
	}
	mergeSortRecursive(labworks, 0, n);
	long long current_day = 0, balls = 0;
	for (int i = 0; i < n; i++) {
		current_day += labworks[i].first;
		balls += labworks[i].second - current_day;
	}
	std::cout << balls;
}